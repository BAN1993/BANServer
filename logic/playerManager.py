...
def addPlayer(conn,numid):
	...
	if playerList.has_key(numid) :
		logging.warning("numid="+str(numid)+",is in list,close it,conn="+str(playerList[numid]))
		...
		if playerList[key].getConn() == conn:
			for key2 in playerList:
				if key2 != key:
					...
					logging.debug("numid="+str(playerList[key].getNumid())+" send numid="+str(playerList[key2].getNumid()))
					playerList[key].senddata(data)
		else:
			data = report.pack(numid)
			playerList[key].senddata(data)
			logging.debug("numid="+str(playerList[key].getNumid())+" send numid="+str(numid))
