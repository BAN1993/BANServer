...
class Server(object):
	...
	def __init__(self, host, port, timeout=2, listennum=5):
		...
		try:
			...
			log.logi(sys._getframe(),"__init__:bind success:host="+str(server_host))
		except :
			raise
		...
		while True:
			...
			for s in readtable:
				if s is self.m_server:
					conn, addr = s.accept()
					log.logi(sys._getframe(),"run:accept="+str(addr))
					conn.setblocking(0)
					...
					try:
						...
						log.loge(sys._getframe(), 'run:Recv Error Code : ' + str(msg[0]) + ' Message ' + msg[1])
					if data:
						ret,xyid,packlen = base.getHand(data)
						if ret == True:
							log.logd(sys._getframe(), "run:len="+str(packlen)+",by="+base.getBytes(data[0:packlen]))
							selectProtocol.getXY(s,xyid,packlen,data[0:packlen])
						#else:
							#log.loge(sys._getframe(), "can not getHand,data="+base.getBytes(data))
					else:
						log.logi(sys._getframe(), "run:client colse:"+str(self.m_clientList[s]))
			...
			for s in writable:
				...
				try:
					...
					log.logd(sys._getframe(), "run:Output Queue is Empty!conn="+self.m_clientList[s])
					#self.m_outputs.remove(s)
				except Exception, e:
					log.loge(sys._getframe(), "run:Send Data Error! ErrMsg:%s" % str(e))
				else:
					try:
						s.sendall(next_msg)
						log.logd(sys._getframe(),"run:send data:addr=%s,databytes=%s" % (str(s.getpeername()),base.getBytes(next_msg)))
					except Exception, e:
						log.loge(sys._getframe(), "run:Send Data to %s  Error And Close! ErrMsg:%s" % (str(s.getpeername()), str(e)))
						playerManager.delPlayerByConn(s)
			## for s in writable:
			
			for s in exceptional:
				log.loge(sys._getframe(), "run:Client:%s Close Error." % str(self.m_clientList[cli]))
				if s in self.m_inputs:
					self.m_inputs.remove(s)
					s.close()
				del self.m_clientList[s]
